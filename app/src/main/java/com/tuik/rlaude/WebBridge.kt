...
class WebBridge(
    ...
    /** Binary-safe file read for the download flow — see readFile for the text-editor path. */
    @JavascriptInterface
    fun readFileBase64(request: String): String = call {
        val json = JSONObject(request)
        files.readFileBase64(json.getString("projectId"), json.getString("path"))
    }

    /** Saves base64 bytes (a generated file from chat, or a project file) into the device's real Downloads folder. */
    @JavascriptInterface
    fun saveFile(request: String): String = call {
        val json = JSONObject(request)
        val bytes = android.util.Base64.decode(json.getString("content"), android.util.Base64.DEFAULT)
        Downloads.save(webView.context, json.getString("name"), json.optString("mime", "application/octet-stream"), bytes)
    }
    ...
    fun sendMessage(request: String): String {
        return try {
            ...
            scope.launch {
                try {
                    val result = openCode.sendMessage(knownSession, projectId, text) { partial ->
                        emit("chunk", JSONObject().put("requestId", requestId).put("sessionId", currentSession ?: knownSession ?: "").put("text", partial))
                    }
