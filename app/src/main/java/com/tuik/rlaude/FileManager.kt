...
class FileManager(context: Context) {
    ...
    /** Binary-safe read (base64) for downloading files that aren't valid UTF-8 text — zips, images, etc. */
    fun readFileBase64(projectId: String, relative: String): JSONObject {
        val file = resolve(projectDir(projectId), relative)
        if (!file.isFile) throw IOException("File not found")
        if (file.length() > 25_000_000) throw IOException("File is larger than the 25 MB download limit")
        val encoded = android.util.Base64.encodeToString(file.readBytes(), android.util.Base64.NO_WRAP)
        return JSONObject().put("path", relative).put("content", encoded).put("size", file.length())
    }
